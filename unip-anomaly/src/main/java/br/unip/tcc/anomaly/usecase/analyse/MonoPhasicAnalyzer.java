...
public class MonoPhasicAnalyzer implements Analyser {
	...
	public void execAnalyse(KeepAliveDTO dto) {
		...
		if (dto.getCurrent().getA() < (configs.getImin() * 0.50)
				|| dto.getVoltage().getAb() < (configs.getVmin() * 0.5)) {
			isShutdown = true;
		}
		// Finaliza se nenhuma anomalia foi detectada, ou adiciona uma leitura
		// normalizada caso detectado anomalia anteriormente
		if (!(isVoltageOverload || isCurrentOverload || isUnderVoltage || isUnderCurrent || isShutdown)) {
			...
			if (anomaly.getStatusid().equals(AnomalyStatusEnum.NORMALIZED)
					|| anomaly.getStatusid().equals(AnomalyStatusEnum.TRATED)) {
				Integer qtd = toNormalize.get(dto.getEquipment().getId()) != null
						? toNormalize.get(dto.getEquipment().getId())
						: 0;
				qtd += 1;
				...
				Integer qtd = Optional.ofNullable(toNormalize.get(dto.getEquipment().getId())).orElse(0);
				qtd += 1;
				toNormalize.put(dto.getEquipment().getId(), qtd);
		...
		if (isShutdown) {
			anomalyType = anomalyTypeRepository.findById(AnomalyTypeEnum.SHUTDOWN.getValor()).get();
			description.append("Equipamento está desligado");
		}
			...
			if (isVoltageOverload) {
				...
				description.append("Tensão medida acima da configurada. Medida: " + dto.getVoltage().getAb()
						+ "/ Configurada: " + configs.getVmax() + "\n");
			} else if (isUnderVoltage) {
				anomalyType = anomalyTypeRepository.findById(AnomalyTypeEnum.VOLTAGEUNDER.getValor()).get();
				description.append("Tensão medida abaixo da configurada. Medida: " + dto.getVoltage().getAb()
						+ "/ Configurada: " + configs.getVmax() + "\n");
			}

			if (isCurrentOverload) {
				anomalyType = anomalyTypeRepository.findById(AnomalyTypeEnum.CURRENTOVERLOAD.getValor()).get();
				description.append("Corrente medida acima da configurada. Medida: " + dto.getCurrent().getA()
						+ "/ Configurada: " + configs.getImax() + "\n");
			} else if (isUnderCurrent) {
				anomalyType = anomalyTypeRepository.findById(AnomalyTypeEnum.CURRENTUNDER.getValor()).get();
				description.append("Corrente medida abaixo da configurada. Medida: " + dto.getCurrent().getA()
						+ "/ Configurada: " + configs.getImin() + "\n");
			}
		}
